...
# Visualization
dtOut = 100.0  # output step length
...
def calcEnthalpyPorosity(T, Tm, dTm, L, c, Cmush):
...
def solveMomentumEquation(u, v, un, vn, dt, dx, dy, nu, beta, T, Tref, g, B):
...
def solvePoissonEquation(p, pn, b, rho, dt, dx, dy, u, v, nit, pWall,
                         beta, g, T, solver, CP, CW, CE, CS, CN, A):
...
#        dt = dt0
    ...
    [f, pc, cMod, B] = calcEnthalpyPorosity(T, Tm, dTm, L, c, Cmush)
    ...
    p = solvePoissonEquation(p, pn, b, rho, dt, dx, dy, u, v, nit, pWall,
                             beta, g, T, 'iterative', CP, CW, CE, CS, CN, A)
